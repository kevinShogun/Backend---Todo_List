...
exports.obtenerTodoList = async (req, res) => {
    try {
        const todoLists = await TodoList.find({ creador: req.usuario.id }).sort({ creado: -1 });
        res.json({ todoLists });
    } catch (error) {
        console.log(error);
        res.status(500).send('Hubo un error');
...
exports.actualizarTodoList = async (req, res) => {
    ...
    try {
        ...
        // actualizar
        todoList = await TodoList.findByIdAndUpdate({ _id: req.params.id }, { $set : nuevoTodoList}, { new: true });
...
exports.eliminarTodoList = async (req, res ) => {
    try {
        ...
        res.json({ msg: 'TodoList eliminado '})
